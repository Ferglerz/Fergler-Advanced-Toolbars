...
function Helpers.rgbaToHex(rgba)
    ...
    return string.format("#%02X%02X%02X%02X", 
        rgba.r, rgba.g, rgba.b, rgba.a or 255)
end
...
function Helpers.getValueDifference(color1, color2)
    if not color1 or not color2 then return 0 end
    
    local rgba1 = Helpers.hexToRGBA(color1)
    local rgba2 = Helpers.hexToRGBA(color2)
    
    if not rgba1 or not rgba2 then return 0 end
...
function Helpers.applyValueDifference(baseColor, valueDiff)
    if not baseColor or not valueDiff then return baseColor end
    local rgba = Helpers.hexToRGBA(baseColor)
    if not rgba then return baseColor end
    
    local hsv = Helpers.rgbToHSV(rgba)
    if not hsv then return baseColor end
    ...
    if not newRGB then return baseColor end
...
function Helpers.getDerivedColors(baseColor, config)
    ...
    local defaultActiveDiff = Helpers.getValueDifference(
        configBaseColor,
        configActiveColor
    )
    
    -- Ensure baseColor is hex string
    local baseColorHex = type(baseColor) == "number" and 
        Helpers.numberToHex(baseColor) or baseColor
...
function Helpers.hexToImGuiColor(hex)
    ...
    if not hex then return 0xFFFFFFFF end  -- Default to white if nil
