...
local function push_font_size(ctx, font_size)
...
--- Compact readout: full-size single line when it fits; else half-size font split across two lines.
function Drawing.measureCompactReadout(ctx, text, max_width, opts)
    opts = opts or {}
    text = text or ""
    max_width = max_width or 0
    local base_size = CONFIG.SIZES.TEXT or 12
    local compact_size = math.max(6, math.floor(base_size * (opts.font_scale or 0.5) + 0.5))
    if max_width < 4 or text == "" then
        return { lines = { text }, font_size = base_size, width = 0, compact = false }
    end
    local full_w = reaper.ImGui_CalcTextSize(ctx, text) or 0
    ...
    for _, line in ipairs(lines) do
        max_w = math.max(max_w, reaper.ImGui_CalcTextSize(ctx, line) or 0)
    end
    if pushed then
        reaper.ImGui_PopFont(ctx)
    end
    return { lines = lines, font_size = compact_size, width = max_w, compact = true }
end

function Drawing.drawCompactReadout(ctx, coords, draw_list, rel_x, rel_y, span_width, height, lines, font_size, text_color, opts)
    opts = opts or {}
    lines = lines or {}
    if #lines < 1 then
        return
    ...
    for i, line in ipairs(lines) do
        local tw = reaper.ImGui_CalcTextSize(ctx, line) or 0
        local tx = rel_x + (span_width - tw) / 2
        local ty = y0 + (i - 1) * (line_h + line_gap)
        Drawing.drawTextRelative(coords, draw_list, tx, ty, text_color, line)
    end
    if pushed then
        reaper.ImGui_PopFont(ctx)
    end
...
-- Toolbar button icon fonts use HEIGHT/2 - SIZE/4 (Renderers/04_Content.lua).
Drawing.ICON_CAP_OFFSET_DIVISOR = 4
Drawing.ICON_GLOBAL_Y_OFFSET = 1

--- Same vertical rule as toolbar button icon fonts; pass the chip band height and pushed icon size.
function Drawing.centeredIconRelY(rel_y, band_height, icon_sz, y_offset)
    return rel_y + band_height / 2 - icon_sz / Drawing.ICON_CAP_OFFSET_DIVISOR + (y_offset or 0) + Drawing.ICON_GLOBAL_Y_OFFSET
end
...
function Drawing.drawCenteredIcon(ctx, coords, draw_list, rel_x, rel_y, width, height, font, icon_char, icon_sz, icon_color, y_offset)
    ...
    local tx = rel_x + (width - tw) / 2
    local ty = Drawing.centeredIconRelY(rel_y, height, icon_sz, y_offset)
    Drawing.drawTextRelative(coords, draw_list, tx, ty, icon_color, icon_char)
    reaper.ImGui_PopFont(ctx)
end

--- Width of an icon-only chip (glyph + horizontal pad). Returns nil when icon font unavailable.
function Drawing.measureIconChipWidth(ctx, icon_font, icon_char, pad_h, icon_sz)
    if not ctx or not icon_font or not icon_char then
        return nil
    end
    if not ensureIconFontAttachedToContext(ctx, icon_font) then
        return nil
    end
    pad_h = pad_h or 4
    icon_sz = icon_sz or CONFIG.SIZES.HEIGHT * 0.5
    reaper.ImGui_PushFont(ctx, icon_font, icon_sz)
    local gw = reaper.ImGui_CalcTextSize(ctx, icon_char)
    reaper.ImGui_PopFont(ctx)
    gw = math.max(gw, icon_sz * 0.65)
    return gw + pad_h * 2
end

--- Icon chip foreground with text fallback when the icon font is missing.
function Drawing.drawIconChipForeground(ctx, coords, draw_list, chip, icon_font, icon_char, icon_sz, text_col, label_text)
    if icon_font and icon_char and ensureIconFontAttachedToContext(ctx, icon_font) then
        Drawing.drawCenteredIcon(ctx, coords, draw_list, chip.x, chip.y, chip.w, chip.h, icon_font, icon_char, icon_sz, text_col, 0)
        return
    end
    Drawing.drawCenteredText(ctx, coords, draw_list, chip.x, chip.y, chip.w, chip.h, label_text or "", text_col, 0)
end
...
function Drawing.drawWidgetPillArrowChip(ctx, coords, draw_list, chip, label, btn_txt, btn_bg, opts)
    ...
    if show_label == "" then
        Drawing.drawCenteredText(ctx, coords, draw_list, chip.x, chip.y, chip.w, chip.h, arrow, txt_col, opts.text_y_offset)
        return
    end
...
function Drawing.drawWidgetPillChipLeadingIcon(ctx, coords, draw_list, chip, text, btn_txt, btn_bg, opts)
    ...
    if icon_w > 0 then
        local ix = start_x
        local icon_ty = Drawing.centeredIconRelY(chip.y, chip.h, icon_sz, opts.icon_y_offset or 0)
        reaper.ImGui_PushFont(ctx, icon_font, icon_sz)
        Drawing.drawTextRelative(coords, draw_list, ix, icon_ty, text_col, icon_char)
        reaper.ImGui_PopFont(ctx)
