...
-- Borderless, non-interactive floating-window flags shared by slide-out panels.
-- Pass no_background = true to also suppress the window background fill (chip draws its own).
function M.slideOutWindowFlags(no_background)
    ...
    if no_background and reaper.ImGui_WindowFlags_NoBackground then
