...
local function title_font_size()
    return (CONFIG and CONFIG.SIZES and CONFIG.SIZES.TITLES_TEXT) or 10
end

local function push_title_font(ctx)
    local font = reaper.ImGui_GetFont(ctx)
    if font then
        reaper.ImGui_PushFont(ctx, font, title_font_size())
        return true
    end
    return false
end

local function pop_title_font(ctx, pushed)
    if pushed then
...
function M.measure(ctx, widget, width, is_vertical)
    ...
    local pushed = push_title_font(ctx)
