...
local WIDGET_DRAW = require("Utils.Widget.common_draw")
...
local function sliderTooltipFineMode(ctx, widget)
...
local function showSliderDragTooltip(ctx, widget)
    ...
    reaper.ImGui_Text(ctx, UTILS.formatWidgetValue(widget, nil, {
        tooltip = true,
        fine_mode = sliderTooltipFineMode(ctx, widget),
    }))
...
function M.slider(ctx, widget, coords, draw_list, rel_x, rel_y, render_width, render_height, text_color, bg_color, is_disabled, preview_mode, layout)
    ...
    if show_value_on_toolbar then
        WIDGET_DRAW.drawSliderWidgetValueAndLabel(ctx, coords, draw_list, widget, sx, sy, sw, text_color, bg_color)
    end
    ...
    if not preview_mode then
        ...
        showSliderDragTooltip(ctx, widget)
    end
end
...
function M.knob(ctx, widget, coords, draw_list, rel_x, rel_y, render_width, render_height, text_color, bg_color, is_disabled, preview_mode, style, bg_only)
    ...
    if not bg_only then
        ...
        if not widget.slider_drag_tooltip then
            ...
            WIDGET_DRAW.drawSliderWidgetValueAndLabel(ctx, coords, draw_list, widget, text_area_x, rel_y + flag_y, text_area_w, text_color, value_bg_color, flag_band_h, flag_text_pad)
        end
