...
local COORDINATES = require("Utils.Core.coordinates")
local COLOR_UTILS = require("Utils.color_utils")
local UTILS = require("Utils.Core.utils")
...
function SlideOutManager.update_animation(widget, host_button, main_hovered)
    ...
    if COLOR_UTILS.slideOutBlocked(widget) then
        if st.t > 0.0 then
            close_slide_state(st)
            if active_slide_key == state_key then
    ...
    -- A widget may gate the open trigger to a sub-region (e.g. one chip) by setting
    -- _slide_hover_gate each frame; when present it replaces whole-button hover.
    ...
    local trigger_hover = main_hovered
    if widget._slide_hover_gate ~= nil then
        trigger_hover = widget._slide_hover_gate
    end
    local is_hovered = not not (trigger_hover or st.hover_inside or widget._st_overlay_focused or popup_open)

    if is_hovered and state_key ~= active_slide_key then
        close_other_slides(state_key)
        active_slide_key = state_key
    end

    -- 200ms slide both ways; 0.5s hold before collapsing after un-hover.
    ANIM.advance_t(st, is_hovered, dt, now, 0.5, 0.2)

    if not is_hovered and st.t <= 0.0 and active_slide_key == state_key then
        active_slide_key = nil
    end
...
--- Optional widget.slide_out_anchor(...) → ax, ay, aw, ah in widget-relative coords.
--- When returned, panel is centered on that rect (e.g. triggering chip) instead of the whole widget.
local function resolve_slide_anchor(widget, ctx, coords, rel_x, rel_y, render_width, render_height, layout)
    local fn = widget and widget.slide_out_anchor
    if type(fn) ~= "function" then
        return rel_x, rel_y, render_width, render_height
    end
    local ok, ax, ay, aw, ah = pcall(fn, widget, ctx, coords, rel_x, rel_y, render_width, layout)
    if ok and ax and ay and aw and ah and aw > 0 and ah > 0 then
        return ax, ay, aw, ah
    end
    return rel_x, rel_y, render_width, render_height
end
...
function SlideOutManager.render(ctx, widget, host_button, rel_x, rel_y, render_width, render_height, coords, draw_list, text_color, bg_color, border_color, layout, render_contents_callback)
    ...
    local anchor_x, anchor_y, anchor_w, anchor_h = resolve_slide_anchor(
        widget, ctx, coords, rel_x, adj_rel_y, render_width, adj_render_height, layout
    )

    local bx1, by1 = coords:toScreen(anchor_x, anchor_y)
    local bx2 = bx1 + anchor_w
    local by2 = by1 + anchor_h
    ...
    if is_vert then
        ...
        -- Center slide-out vertically on anchor rect (chip or whole widget)
        win_y = by1 + (anchor_h - H_slideout) * 0.5
        ...
        local centered = bx1 + (anchor_w - W_slideout) * 0.5
        win_x = math.max(vpx, math.min(centered, vpx + vpw - W_slideout))
    end
    ...
    local eased_t = ANIM.ease_in_out(st.t)
    local fade_alpha = ANIM.fade_alpha(st.t)
    ...
    if visible then
        ...
        if fade_alpha > 0.0 then
            ...
            local panel_bg = bg_color
            if BUTTON_UTILS and not BUTTON_UTILS.hasAlpha(panel_bg) then
                panel_bg = COLOR_UTILS.toImGuiColor(CONFIG.COLORS.NORMAL.BG.NORMAL)
            end
            if fade_alpha < 1.0 then
                panel_bg = COLOR_UTILS.applyAlphaFactor(panel_bg, fade_alpha)
            end
            local border = border_color
            if border and fade_alpha < 1.0 then
                border = COLOR_UTILS.applyAlphaFactor(border, fade_alpha)
            end

            local win_x, win_y = reaper.ImGui_GetWindowPos(ctx)
            local bx1 = win_x + slide_x
            local by1 = win_y + slide_y
            local bx2 = bx1 + W_slideout
            local by2 = by1 + H_slideout
            reaper.ImGui_DrawList_AddRectFilled(w_dl, bx1, by1, bx2, by2, panel_bg, button_rounding, 0)
            if border then
                reaper.ImGui_DrawList_AddRect(w_dl, bx1, by1, bx2, by2, border, button_rounding, 0, 1)
            end
...
function SlideOutManager.close_for_widget(widget, host_button)
    local st, state_key = get_slide_state(host_button)
    if not st then
        return
    end
