...
- At runtime, each per-icon font exposes its glyph at a **single fixed code point: U+0021 (`'!'`)**. A **fontTools** pipeline (below) subsets each source glyph and **remaps the cmap** so every exported font encodes its icon at `0x21`. The **name comes from the output filename**, not from enumerating Unicode ranges inside a category font.

Persisted data can remain **`icon_font` + `icon_char`**: for per-icon fonts, **`icon_char` is always `"!"`** (optional later: omit `icon_char` and default to `"!"` in code).
...
| Per-icon (new) | One `.ttf` per icon after split + remap | Fixed **`"!"`** (U+0021) for every per-icon font |
...
| Area | Behavior |
|------|----------|
| `Advanced Toolbars.lua` | Scans `IconFonts/*.ttf`, parses `_*digits*.ttf` for glyph count, builds `ICON_FONTS[]` with `path`, `display_name`, `icon_range` from U+00C0 (legacy). Per-icon mode uses a single slot at **U+0021**. |
...
2. **`ICON_FONTS` entries:** Each entry represents **one icon**, not one category. Fields might include: `path`, `icon_id` or `name` (from filename), `display_name` (formatted for UI), and `icon_range` **{ start = 0x21, laFin = 0x21 }** or a shared constant **`ICON_UNICODE = 0x21`** used when rendering and in the selector.
...
4. **Rendering:** Push the correct per-icon ImGui font; draw **`"!"`** (or `utf8(0x21)`) for that font. No per-button glyph variation for per-icon mode—identity is **`icon_font` path** (and optional display name from stem).
...
**Role:** Take an existing **category font** (many glyphs) and produce **one `.ttf` per glyph**, each with that glyph moved to **U+0021** so the Reaper script can treat every icon font identically at runtime.
...
# Remap surviving glyph to U+0021 on all cmap subtables
for table in new_font["cmap"].tables:
    table.cmap = {0x21: glyph_name_after_subset}
...
**Validation:** Open a few outputs in a font viewer or re-load with `TTFont` and confirm `getBestCmap()` is `{33: '<glyphname>'}`.
...
- For per-icon files, set `icon_range` to **one code point** (start == end == **0x21**) matching the **fontTools remap**. Do **not** use U+00C0 for new assets unless you keep a legacy branch.
- When saving buttons that use per-icon fonts, **`icon_char` can always be `"!"`**; migration may set this automatically when loading old configs that pointed at multi-glyph fonts.
...
- Document **pip / fonttools** for maintainers: `requirements.txt`, how to run the split script, and that **glyph address for icons is `!` (U+0021)**.
...
| Per-icon font cmap not exactly `{33: glyph}` | Standardize on the **fontTools** pipeline; validate outputs before shipping. |
...
The restructure is **wiring, UX, and a fixed icon encoding**: **fontTools** splits category fonts into **one `.ttf` per icon** with each glyph remapped to **`'!'` (U+0021)**. The script is maintained with **`pip install fonttools`**. At runtime, one canonical scanner builds **`ICON_FONTS` as a flat list of named icons**; the selector picks by **filename stem**; rendering uses **`icon_font` + `"!"`** for per-icon entries. Legacy multi-glyph fonts can remain on **U+00C0** until migrated. Serialized **`icon_font` + `icon_char`** stays compatible if per-icon buttons store **`icon_char = "!"`** (or default it in code).
...
2. Keep `icon_char` forever or migrate to optional field with default **`"!"`** for per-icon mode?
