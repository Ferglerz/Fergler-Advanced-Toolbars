...
function SettingsWindow:render(ctx, saveCallback, toggleColorEditor, toggleDocking)
    ...
    local rounding_changed, new_rounding = 
        self:renderSettingsRow(ctx, "Button Rounding", self.r.ImGui_SliderInt, "##rounding", CONFIG.SIZES.ROUNDING, 0, 30)
    ...
    local width_changed, new_width = 
        self:renderSettingsRow(ctx, "Min Button Width", self.r.ImGui_SliderInt, "##minwidth", CONFIG.SIZES.MIN_WIDTH, 20, 200)
    ...
    local separator_changed, new_separator_width =
        self:renderSettingsRow(ctx, "Separator Width", self.r.ImGui_SliderInt, "##separator", CONFIG.SIZES.SEPARATOR_WIDTH, 4, 50)
    ...
    local scale_changed, new_scale =
        self:renderSettingsRow(ctx, "Image Icon Scale", self.r.ImGui_SliderDouble, "##iconscale", CONFIG.ICON_FONT.SCALE, 0.1, 2.0, "%.2f")
    ...
    local size_changed, new_size =
        self:renderSettingsRow(ctx, "Built-in Icon Size (must restart)", self.r.ImGui_SliderInt, "##iconsize", CONFIG.ICON_FONT.SIZE, 4, 18)
