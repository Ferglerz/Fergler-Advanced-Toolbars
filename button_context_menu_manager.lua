...
function ButtonContextMenuManager:handleButtonRename(button, saveConfig)
    ...
    local retval, new_name =
        self.r.GetUserInputs("Rename Toolbar Item", 2, "Top Line:,Bottom Line:,extrawidth=100", top_line .. "," .. bottom_line)
    if not retval then
        return false
    end
        ...
        table.insert(menu_items, {
            ...
            fn = function()
                local retval, new_name = self.r.GetUserInputs(
                    "Group Name", 1, "Group Name:,extrawidth=100", active_group.label.text or "")
                if retval then
